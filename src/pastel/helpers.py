...
def load_images_from_directory(image_dir: str | Path) -> list[BaseImage]:
    return [
        BaseImage(image_path=image_path, image=load_image(image_path))
        for image_path in pathlib.Path(image_dir).glob("*.png")
    ]
